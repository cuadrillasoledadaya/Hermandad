...
**Herramienta para crear iconos maskable**: https://maskable.app/
...
- Índices secundarios: email, numero_hermano, activo (hermanos); hermano_id, fecha (pagos)
...
- Actualmente los mutations usan Supabase directamente sin pasar por el sistema offline
...
9. ✅ `src/hooks/use-network-status.ts` - Detección de red
10. ✅ `src/hooks/use-offline-sync.ts` - Sincronización offline
11. ✅ `src/components/ui/offline-banner.tsx` - Banner interactivo
12. ✅ `src/lib/offline-mutation.ts` - Utilidad mutations
13. ✅ `src/components/ui/skeleton.tsx` - Skeletons reutilizables
14. ✅ `src/app/loading.tsx` - Loading global
15. ✅ `src/app/error.tsx` - Error boundary
...
4. **Actualizar hooks de mutations** (IMPORTANTE)
...
**Tu aplicación está lista para el 95% de casos de uso.** Solo necesitas los iconos y conectar el sistema offline a los hooks reales.
